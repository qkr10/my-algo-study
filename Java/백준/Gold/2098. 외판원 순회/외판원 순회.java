...
#### 내 문제점 목록
1. dfs 를 사용하면 일단 dp 부터 떠올려야 함.
2. dfs 반환값에 덧셈을 하는데도, minCost 변수 초기값을 int 최대값으로 줌.
3. 재귀 탈출 조건에서 node == 0 을 빼먹음. node == 0 이 없을경우, 아래와 같은 테케에서 틀림.
```
3
0 1 1
1 0 10
...
answer = 12
```
4. if (node == 0 && visit == maxVisit) return 0; 만으로는 위 테케를 통과할수 없음.
    - 0에서 출발/도착하도록 강제하는 방법이 궁금함.

#### 풀이과정
1. N, graph[N][N] 입력받음
2. visit[i] = (i번째 노드를 방문했다면 true), minCost = (현재까지 순회하는 비용의 최소값) 변수 초기화
3. dfs(n, cost) : dfs와 백트래킹으로 답을 찾아서 minCost 변수에 저장함.
    - 이미 minCost 보다 cost 가 크다면 현재 노드는 건너뜀.
